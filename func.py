...
def pptx_work(presentation, name, mobile, code,date, sip, aror, figi, avg_capacities, avg_tolerances,
    ...
    fill_bio(mypres,1,14,1,2, str(format_currency(int(monthly_sip*nifty),'INR', locale='en_IN')))
    fill_bio(mypres,1,14,2,1, str(it*100)+"%")
    fill_bio(mypres,1,14,2,2, str(format_currency(int(monthly_sip*it),'INR', locale='en_IN')))
    fill_bio(mypres,1,14,3,1, str(smallCap*100)+"%")
    fill_bio(mypres,1,14,3,2, str(format_currency(int(monthly_sip*smallCap),'INR', locale='en_IN')))
    fill_bio(mypres,1,14,4,1, str(longDurationBond*100)+"%")
    fill_bio(mypres,1,14,4,2, str(format_currency(int(monthly_sip*longDurationBond),'INR', locale='en_IN')))
    fill_bio(mypres,1,14,5,1, "100%")
    fill_bio(mypres,1,14,5,2, str(format_currency(int(monthly_sip),'INR', locale='en_IN')))
    ...
    mypres.slides[1].shapes[9].text_frame.paragraphs[0].text = 'A monthly SIP amount of '+str(format_currency(int(sip/12), 'INR', locale='en_IN'))+' is recommended based on your current lifestyle and commitments. However, the more the better.'
    for paragraph in mypres.slides[1].shapes[9].text_frame.paragraphs:
        paragraph.font.size = Pt(10)
        paragraph.font.name = 'Times New Roman'
        
    mypres.slides[1].shapes[16].text_frame.paragraphs[0].text = 'If you START investing and stick to the plan of monthly SIP of '+str(format_currency(int(sip/12), 'INR', locale='en_IN'))+ ' and earn avg. annual return of '+str(round(nif*100,1))+'% on equity portion and '+str(round(figi*100,1))+' % on debt portion , this is how your portfolio will grow- '
    for paragraph in mypres.slides[1].shapes[16].text_frame.paragraphs:
        paragraph.font.size = Pt(10)
        paragraph.font.name = 'Times New Roman'    
    ...
    mypres.slides[2].shapes[7].text_frame.paragraphs[0].text = 'SRN: '+ str(srn)+ '\nTransaction ID: '+str(int(mobile)-1000)+'\nReport Created:  '+str(date)
    for paragraph in mypres.slides[2].shapes[7].text_frame.paragraphs:
        paragraph.font.size = Pt(8)
        paragraph.font.name = 'Times New Roman'
